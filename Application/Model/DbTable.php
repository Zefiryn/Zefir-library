...
class Zefir_Application_Model_DbTable extends Zend_Db_Table_Abstract
{
	 ...
	 */
  	public function __construct($config = array())
		...
		//for mysql get Zefir adapter in order to allow PHP Quick Profiler to function
		if ($options['resources']['db']['adapter'] == 'PDO_MYSQL') 
		{
				if (!Zend_Registry::isRegistered('adapter'))
				{
					//create and register adapter object
					$adapter = new Zefir_Db_Adapter_Pdo_Mysql($options['resources']['db']['params']);
					Zend_Registry::set('adapter', $adapter);
				}
				else 
				{
					//load adpater for this class; only one is needed per application call
	 ...
	 * 
	 * @deprecated use findData instead as it offers better functionality
	 * @access public
	 * @param array $data
	...
	{
		/**
		* @todo add ability to count parents through join table
		 */
		$select = $this->select()->from(array($this->getTablename()), array('number' => 'count(*)'))->where($refColumn.'=?', $refValue);
